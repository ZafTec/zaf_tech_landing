...
export const POST: APIRoute = async ({ request }) => {
  ...
  const techStack = (payload.tech_stack ?? "").trim() || null;

  if (!name || !email || !building || !problem) {
    return new Response(
  ...
  await db`
    INSERT INTO audit_submissions (id, name, email, building, problem, tech_stack, user_agent, ip_address, created_at)
    VALUES (${id}, ${name}, ${email}, ${building}, ${problem}, ${techStack}, ${userAgent}, ${ipAddress}, ${createdAt});
  `;
